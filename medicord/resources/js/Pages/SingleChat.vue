...
const props = defineProps(['receiverId', 'userId', 'sentMessages', 'receivedMessages', 'allMessages'])
let message = ''
...
// fix per non duplicare gli ascoltatori di eventi, non l'ho capito del tutto
window.Echo.private(`user.${channelId}`).stopListening('NewMessageSent');
...
// 🧭 sorting messages for hour and data
let messagesArr = [];
for (const e in props.allMessages) {
  messagesArr.push(props.allMessages[e]);
}
(messagesArr).sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
//sendind message to back-end 
const sendMessage = () => {
...
<template>
  <!-- <h1>Chat con l'utente {{ receiverId }}</h1>

  <ul>
    <li v-for="singleMessage in messagesArr" :class="singleMessage.sender_id == user.id ? 'bg-red-600 m-2 p-2' : 'text-right bg-blue-400 p-2'">
      <strong>User: {{ singleMessage.sender_id }}(io) dice -></strong>{{ singleMessage.message }}
  ...
  <div class="flex ">
    <!-- Sidebar -->
    <div class="hidden  lg:block w-1/4 bg-white border-r border-gray-300">
      <!-- Sidebar Header -->
    ...
    <div class="flex-1">
      ...
      <header class="bg-white p-4 text-gray-700">
        <h1 class="text-2xl font-semibold">chat with user {{ receiverId }}</h1>
      </header>
      <div v-for="singleMessage in messagesArr">
        <div v-if="singleMessage.sender_id == user.id">
          <!-- Outgoing Message -->
        ...
        <div v-else>
          ...
          <div class="flex mb-4 cursor-pointer">
            ...
            <div class="flex max-w-96 bg-white rounded-lg p-3 gap-3">
              <p class="text-gray-700">{{ singleMessage.message }}</p>
            </div>
          </div>
        </div>
      ...
      <!-- Chat Input -->
      <footer class="bg-white block border-gray-300 p-4">
        <div class="flex items-center ">
          <input v-model="message" name="message" id="message" type="text" placeholder="Type a message..." class="w-full p-2 rounded-md border border-gray-400 focus:outline-none focus:border-blue-500">
